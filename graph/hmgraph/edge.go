...
type Edge struct {
	...
	// position of this edge in each endpoint's edge slice
	vertexIndices [2]int
}
...
// Index returns the edge's graph-global index (not stable under graph modifications!)
func (edge *Edge) Index() int {
	return edge.index
}
...
// String returns a formatted representation of the edge including edge map information.
func (edge *Edge) String() string {
	...
	if len(edge.vertices[0].graph.edgeMaps) > 0 {
		...
		for _, em := range edge.vertices[0].graph.edgeMaps {
			result += fmt.Sprint(em.Label() + "=" + em.AsString(edge) + ";")
		}
		result += ")"
	}
