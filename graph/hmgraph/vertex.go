...
// A VertexVisitor is a function for visiting vertices
...
// OutArcs returns a slice containing all outgoing arcs
...
func (vertex *Vertex) OutArcs() []*Arc {
...
// InArcs returns a slice containing all incoming arcs
// without giving access to the underlying structures in O(indeg(v)).
// Please note that the preferred way of iterating over all incoming arcs
// is the ForInArcs function.
func (vertex *Vertex) InArcs() []*Arc {
...
// Edges returns a slice containing all incident edges
...
func (vertex *Vertex) Edges() []*Edge {
	return slices.Clone(vertex.edges)
}

// Index returns the vertex's index (not stable under graph modifications!)
func (vertex *Vertex) Index() int {
	return vertex.index
}

// Graph returns the graph a vertex belongs to.
func (vertex *Vertex) Graph() *Graph {
	return vertex.graph
}
...
func (vertex *Vertex) String() string {
	...
	if len(vertex.graph.vertexMaps) > 0 {
		...
		for _, vm := range vertex.graph.vertexMaps {
			result += fmt.Sprint(vm.Label() + "=" + vm.AsString(vertex) + ";")
		}
		result += ")"
	}
